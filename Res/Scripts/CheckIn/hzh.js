...
function getToken() {
    if ($request && $request.method != 'OPTIONS') {
        const Val = $request.headers['user-token'];
        if (Val) {
            let cookie = $.setdata(Val, token_key)
                if (cookie) {
            ...
            try {
                ...
                if(result?.businessCode == '1000'){
                    if(result?.content.success == true){
                        message += `签到:获得积分:${result?.content.point}\n`;
                    } else if(result?.content.isSign == true){
                        message += `签到:请勿重复签到\n`;
                    }
                } else {
